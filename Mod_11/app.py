...
from Mod_11 import tab2, tab1, tab3
...
def tab3_bar_chanel(start_date, end_date):
    ...
    grouped = truncated[truncated['total_amt'] > 0].groupby([pd.Grouper(key='tran_date'), 'Store_type'])[
        'total_amt'].sum().round(2).unstack()
...
def tab3_barh_chanel_subcat(chosen_cat_chan):
    grouped = df.merged[(df.merged['total_amt'] > 0) & (df.merged['Store_type'] == chosen_cat_chan)].pivot_table(
        index='prod_subcat', columns='Gender', values='total_amt', aggfunc='sum').assign(
        _sum=lambda x: x['F'] + x['M']).sort_values(by='_sum').round(2)
